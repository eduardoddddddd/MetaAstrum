...
import { FirdariaPeriod } from '../types';
...
export default function Firdaria({ periods }: Props) {
  return (
        ...
        {periods.map((p, i) => (
          <div key={i} className="relative pl-8 md:pl-12">
            ...
            <p className="text-sm text-stone-500 max-w-2xl leading-relaxed">
              Fase regida por {p.planet}, activando los temas de la casa {Math.floor(Math.random() * 12) + 1} 
              y la casa {Math.floor(Math.random() * 12) + 1}. Las promesas natales de este planeta se manifiestan 
              principalmente durante estos años.
            </p>
          </div>
        ))}
      </div>
